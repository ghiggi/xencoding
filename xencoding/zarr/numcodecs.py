...
def _get_blosc(algorithm='lz4', clevel=1, shuffle=1, blocksize=0):
    ...
    """
    return numcodecs.blosc.Blosc(cname=algorithm,
                                 clevel=clevel, 
                                 shuffle=shuffle,
                                 blocksize=blocksize,
                                 )
...
def _get_zfpy(mode="fixed_accuracy", tolerance=-1, rate=-1, precision=-1):
    ...
    return numcodecs.zfpy.ZFPY(mode, 
                               tolerance=tolerance, 
                               rate=rate, 
                               precision=precision)
...
def _get_lz4(accelaration=1):
    ...
    """
    return numcodecs.lz4.LZ4(acceleration=accelaration)
